...
void Mezclador_timerCallBack() {
    if (dispararConversion) {
        ...
        ADCON3Lbits.CNVRTCH = 1;    //dispara la conversi�n, que tomara aprox. 1.833uS
    }
...
void Mezclador_conversionCallBack(enum ADC_CHANNEL ch, uint16_t adcVal) {
    ...
    // Coversion a voltaje y divisi�n entre 100 para eliminar la ganancia.
    ...
    // Factor com�n para ambos canales
    ...
    if (ch == (CANAL_OXI - 3)) {
        ...
        return; // Si no coincide con ning�n canal esperado, salir de la funci�n
    }
